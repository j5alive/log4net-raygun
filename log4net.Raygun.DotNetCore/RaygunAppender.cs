...
namespace log4net.Raygun.DotNetCore
{
    public class RaygunAppender : AppenderSkeleton
    {
        ...
        public RaygunAppender() : this(
            new RaygunMessageBuilder(le => new HttpContextAdapter((HttpContext)le.GetProperties()["log4net.Raygun.HttpContext"])),
            RaygunClientFactoryMethod.From(apiKey => new RaygunClientAdapter(new RaygunClient(apiKey))))
        {
        }

        protected RaygunAppender(IRaygunMessageBuilder raygunMessageBuilder, IRaygunClientFactory raygunClientFactory)
        ...
        protected RaygunAppender(IUserCustomDataBuilder userCustomDataBuilder, IRaygunMessageBuilder raygunMessageBuilder,
        ...
        private void SendErrorToRaygun(RaygunMessage raygunMessage)
        {
            try
            {
                Retry.Action(() =>
                {
                    ...
                    var raygunClient = RaygunClientFactory.Create(new RaygunSettings { ApiKey = ApiKey });
        ...
        {
            public const string Tags = "log4net.Raygun.DotNetCore.Tags";
            public const string AffectedUser = "log4net.Raygun.DotNetCore.AffectedUser";
        }
    }
}